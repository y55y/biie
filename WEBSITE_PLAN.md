# BIÉE Gulf Partner Landing Website Plan

## Goal

Build a full landing website for BIÉE New York as the Gulf region brand partner presence. The page should introduce BIÉE, explain the product range, localize the brand for Saudi/GCC customers, and drive shoppers to the official The Ora Beauty store:

https://theorabeauty.com/biie-new-york/brand-1022818369

Primary contacts:

- Email: info@biiesa.com
- WhatsApp/phone: +966556305632

## Source Findings

BIÉE official site positions the brand as "Biie New York" with the tagline "Because you matter." Its visible product categories are face, lips, nails, and eyes, and listed products include Matte Kiss Proof liquid lipstick, mascara, Matte Attack Sugar Lipstick, Color Expert nail polish, and foundation.

The Ora Beauty is a Saudi beauty store focused on makeup, skincare, and body care categories. The BIÉE page should therefore behave as a branded launch page, not a replacement ecommerce catalog.

Competitor cues:

- MAC: bold black-and-white identity, dense product navigation, artist/professional authority, best-seller modules.
- Sephora Middle East: localized retail structure, clear category paths, new arrivals, bestsellers, face/eyes/lips/brows/tools navigation.
- Maybelline: campaign-first hero, "What's New", best sellers, shade counts, tutorials/tips.

Market cue from Brand Finance 2025: cosmetics brand value reached USD 160B; Chanel became the most valuable cosmetics brand, with L'Oreal, Guerlain, Shiseido, Lancome and other premium brands showing that innovation, premium positioning, and desirability matter.

## Design Frame

Purpose:
Convert Gulf beauty shoppers from awareness to The Ora Beauty store visits while making BIÉE feel legitimate, premium, and locally supported.

Audience:
Saudi and GCC makeup buyers, beauty creators, boutique retailers, and customers who discover BIÉE through social commerce or The Ora Beauty.

Tone:
Editorial New York beauty energy softened for Gulf luxury retail. Confident, glossy, feminine, modern, not overly clinical.

Differentiation:
"New York color, Gulf-ready wear." The page should emphasize long-wear, high-impact color, heat-aware daily use, and a direct Saudi buying path.

## Visual Direction

Palette:

- Ink black: #090807
- Warm ivory: #F7F1EA
- BIÉE rose: #B95A67
- Deep berry: #681C2C
- Champagne gold: #C8A45D
- Sandstone neutral: #D8C6B2

Typography:

- Display: Playfair Display or Cormorant Garamond for campaign headlines.
- Body/UI: Manrope or Avenir-like sans for clean ecommerce readability.
- Arabic fallback: IBM Plex Sans Arabic or Noto Kufi Arabic if bilingual copy is added.

Signature detail:
A full-bleed glossy campaign hero with lipstick/foundation product silhouettes crossing a warm Gulf stone backdrop, plus shade-swatch navigation that feels like makeup, not generic buttons.

## Page Structure

1. Header
   - BIÉE logo
   - Links: Collection, Best Sellers, Why BIÉE, For Gulf, Contact
   - Primary CTA: Shop at The Ora Beauty
   - Secondary CTA: WhatsApp

2. Hero
   - H1: BIÉE New York
   - Subhead: New York color, Gulf-ready wear.
   - Copy: Official Gulf partner landing page for expressive makeup, long-wear color, and everyday confidence.
   - CTAs: Shop BIÉE at The Ora Beauty, Contact Gulf Partner
   - Hero image: generated campaign/product scene.

3. Trust Strip
   - Official Gulf partner
   - Saudi support contact
   - Available via The Ora Beauty
   - Face, lips, eyes, nails

4. Collection Overview
   - Lips: Matte Kiss Proof Liquid Lipstick, Matte Attack Sugar Lipstick
   - Face: Foundation
   - Eyes: Mascara
   - Nails: Color Expert Nail Polish

5. Best-Seller Feature
   - Hero product story around Matte Kiss Proof Liquid Lipstick.
   - Swatches, finish notes, occasion-based copy.

6. Gulf Wear Section
   - Heat-aware positioning: long-wear color, polished finish, day-to-night looks.
   - Avoid medical or unsupported claims.

7. Lookbook / Campaign Tiles
   - Soft glam
   - Bold lip evening
   - Everyday office
   - Eid/event-ready shine

8. Brand Story
   - "Because you matter" framed around self-expression, accessible premium beauty, and confidence.

9. Store CTA Band
   - Direct The Ora Beauty link
   - WhatsApp support
   - Email support

10. Footer
   - Contact details
   - Store link
   - Original BIÉE company site link
   - Disclaimer: This Gulf landing page directs purchases to The Ora Beauty.

## Content Style

Use polished English with optional Arabic microcopy later. Avoid unsupported claims such as waterproof, dermatologist-tested, clean, organic, FDA-approved, or cruelty-free unless verified from BIÉE materials.

Sample hero copy:

BIÉE New York
New York color, Gulf-ready wear.
Discover expressive lips, polished skin, defined eyes, and high-gloss nail color from BIÉE New York, available for Gulf shoppers through The Ora Beauty.

## Build Recommendation

Create a responsive static website first:

- `index.html`
- `styles.css`
- `script.js`
- `assets/`

Use local extracted logo/product assets where possible. Use generated campaign images for sections where the catalog does not provide web-ready photography.

After build:

- Verify desktop 1440px, tablet 768px, and mobile 390px.
- Check CTA links.
- Check WhatsApp link format: `https://wa.me/966556305632`
- Check text wrapping and contrast.

